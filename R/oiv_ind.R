...
#' @param force.Hemisphere if NULL (default) detect hemisphere looking at average temperature of January and July. In case of incomplete timeseries could be forced setting parameter to "north" or "south".
#'
...
oiv_ind<-function (daily_Tn=NULL,daily_Tx=NULL,daily_Tm=NULL, daily_P=NULL, first.yr=NULL, last.yr=NULL, subs_missing=TRUE, coeff_HI=1.04, quant_Tn_rest=0.5,quant_Tn_veg=0.5,quant_Tx_veg=0.5, indices=1:10, force.Hemisphere = NULL)

{
  ...
  if(first.yr > last.yr) print("First year greater than last year!", quote=FALSE) else {
    ...
    if(!is.null(force.Hemisphere) & isTRUE(force.Hemisphere=="north")){
        S_emisph <- FALSE
        message("Hemisphere forced to NORTH")
        block <- FALSE
    } else if(!is.null(force.Hemisphere) & isTRUE(force.Hemisphere=="south")){
        S_emisph <- TRUE
        message("Hemisphere forced to SOUTH")
        block <- FALSE
    } else if(is.null(force.Hemisphere)){
        if( anyNA(colMeans(as.data.frame(daily_Tm[daily_Tm$month==1,-(1:3)]), na.rm=T)) | anyNA(colMeans(as.data.frame(daily_Tm[daily_Tm$month==7,-(1:3)]), na.rm=T))) {
            Tm_veg=NA;winkler_mean=NA;BEDD_mean=NA;HI_mean=NA;CNI_mean=NA;FSI_mean=NA;BI_mean=NA;Tn_rest_quantile=NA;Tn_veg_quantile=NA;Tx_veg_quantile=NA
            warning("Average temperature for January and/or July is NA for at least one station. Impossible detection of right hemisphere. Indexes will be NA.")
            block <- TRUE
        } else {
    ...
    if(!block){
        if(S_emisph) index_months<-c(10:12, 1:4) else index_months<-c(4:10)
